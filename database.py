...
conn = psycopg2.connect("dbname=laundry user=postgres password=12345")
cur = conn.cursor()
...
def insert_item(v):
  vs = str(v)
  q = "insert into services(servicetype, itemtype, service_price)"\
    "values" + vs
  cur.execute(q)
  conn.commit()
  return q
...
def update_service(vs):
        ...
        itemtype = vs[2]
        service_price = vs[3]
        q = "UPDATE services SET servicetype = %s, itemtype = %s, service_price = %s WHERE s_id = %s"
        cur.execute(q, (servicetype,itemtype,service_price,s_id))
        conn.commit()
        return q
